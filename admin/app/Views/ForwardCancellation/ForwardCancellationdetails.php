...
<?php foreach ($forwardcoverdetails as $row) : ?> <option value='<?php echo $row['forward_coverdetails_id'] ?>' <?php echo (!empty($query[0]->role_id) && $query[0]->role_id == $row['forward_coverdetails_id']) ? 'selected' : '' ?>><?php echo $row['deal_no'] ?></option> <?php endforeach; ?>
	...
	<input id="cancelledforwardamount<?php echo  $i?>" type="number" name="cancelledforwardamount[]" placeholder="Cancelled Forward Amount (FC)"  class="form-control"  required/>
...
<input id="utilisedforwardamount<?php echo  $i?>" type="number" data-cancid="<?php echo $i?>" name="utilisedforwardamount[]" placeholder="Utilised Forward Amount (FC)"  class="form-control utilisedforwardamount"  value="" required/>

	</div>
<div class="form-group"><label class="form-label"><?php echo $pade_title2 ?></label>

<input id="cancellationrate<?php echo  $i?>" type="text" name="cancellationrate[]" placeholder="Cancellation Rate" class="form-control" required="">
